...
- **`ligolo-ng` and `exploitdb` are built and read out of a tracked branch**, so neither has a version the manifests can hold. The ligolo build is also guarded on all five artifacts `webroot.copy` expects (the linux proxy and the four agents) existing, so a `git pull` that brings in new commits does **not** rebuild — delete `dist/` (or `make clean`) when you want the pulled code. Checking the whole set rather than the proxy alone is what makes a half-finished `make all` retry instead of sticking: `all` builds `linux` first, so the proxy appears before the three cross-built targets have run. The build also runs `git checkout -- '*.go'` afterwards, because every ligolo target depends on upstream's `lint`, which runs `go fmt` in the checkout and would leave `clone_or_pull`'s `--ff-only` pull refusing from then on. exploitdb needs no build step, so its `git pull` is the update.
...
- **Metasploit tracks Rapid7's nightly channel, so no two VMs built on different days carry the same msf.** That is the only distribution Rapid7 offers outside the commercial installer. Module 60 presence-checks it (`have msfconsole ||`) precisely because the suite is rolling: without that, every re-run of the module would pull the current nightly — a ~250MB upgrade mid-engagement. The consequence is that the box keeps whatever nightly it first installed until you upgrade it by hand, and nothing on the box records which one that was. It also arrives with no database: `msfconsole` runs fine, but `db_nmap`, workspaces and the `hosts`/`services` tables need `sudo apt-get install postgresql && msfdb init`, which module 60 deliberately leaves to the operator because it is per-engagement state.
